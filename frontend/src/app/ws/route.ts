...
async function handler(request: NextRequest) {
  try {
    ...
    console.log(`WebSocket root proxy: ${request.method} to: ${url}`);
    console.log(`WebSocket request headers:`, Object.fromEntries([...request.headers.entries()]));
    ...
    if (request.method !== 'GET' && request.method !== 'HEAD') {
      ...
      console.log(`WebSocket request body: ${body}`);
    ...
    // 응답 상태 및 헤더 로깅
    console.log(`WebSocket root proxy response status: ${response.status}`);
    console.log(`WebSocket root proxy response headers: ${JSON.stringify(Object.fromEntries([...response.headers.entries()]))}`);
    
    // 응답 그대로 반환
    const data = await response.text();
    console.log(`WebSocket root proxy response data (first 100 chars): ${data.substring(0, 100)}...`);
    ...
    console.error('WebSocket root proxy error:', error);
