...
public class BusController {
    ...
    private List<Map<String, String>> parseXmlResponse(String xmlString) throws Exception {
        System.out.println("\n=== Starting XML Response Parsing ===");
        System.out.println("Raw XML Response Length: " + xmlString.length());
        System.out.println("First 500 chars of XML: " + xmlString.substring(0, Math.min(500, xmlString.length())));
        ...
        System.out.println("Number of busArrivalList items found: " + itemList.getLength());
        ...
        for (int i = 0; i < itemList.getLength(); i++) {
            ...
            System.out.println("\nProcessing bus - RouteId: " + routeId + ", BusNumber: " + busNumber);
            ...
            if (targetBusNumbers.contains(busNumber)) {
                ...
                System.out.println("First bus info - PredictTime1: " + predictTime1 + ", RemainSeat1: " + remainSeat1);

                // 첫 번째 버스 정보가 유효한 경우에만 처리
                if (!predictTime1.isEmpty()) {
                    ...
                    System.out.println(
                            "Second bus info - PredictTime2: " + predictTime2 + ", RemainSeat2: " + remainSeat2);

                    // predictTime2가 있는 경우 추가
                    if (!predictTime2.isEmpty()) {
                        try {
                            int time1 = Integer.parseInt(predictTime1);
                            int time2 = Integer.parseInt(predictTime2);
                            // 두 번째 버스 시간이 첫 번째보다 크거나, 첫 번째가 매우 큰 경우(다음 운행)
                            if (time2 > time1 || time1 > 120) {
                                System.out.println("Adding second bus info - Valid time found");
                                busInfo.put("도착시간2", predictTime2);
                                busInfo.put("남은좌석수2", remainSeat2.equals("-1") ? "정보 없음" : remainSeat2 + "석 남음");
                            } else {
                                System.out.println("Skipping second bus info - Invalid time difference");
                            }
                        } catch (NumberFormatException e) {
                            System.out.println("Error parsing arrival times: " + e.getMessage());
                        }
                    } else {
                        System.out.println("Skipping second bus info - PredictTime2 is empty");
                    }

                    System.out.println("Final busInfo map: " + busInfo);
                    busInfoList.add(busInfo);
                } else {
                    System.out.println("Skipping bus info - First bus has no valid arrival time");
                }
            }
        }

        System.out.println("\n=== Final busInfoList size: " + busInfoList.size() + " ===");
        System.out.println("Final busInfoList: " + busInfoList);
        return busInfoList;
    }
